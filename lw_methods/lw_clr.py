...
# ----------------- Ensemble function -------------------#
def ensemble(Ta, RH, elev, Qsi=None):
    ...
    lw_ensemble["lclr_satterlund1979"] = satterlund1979(Ta, RH)
...
def vap_pres(Ta, RH):
    ...
    sat_vap_pres = 6.1078 * np.exp(
        (17.2693882 * (Ta - 273.15)) / (Ta - 35.86)
...
def prata1996_w(Ta, RH):
    """from Prata 1996"""
    return (
        (465 * vap_pres(Ta, RH)) / Ta
    )  # note: corrected equation constant to be 465 instead of 4650 (error in Flerchinger Table 1 footnote c)
...
def niemela2001(Ta, RH):
    ...
    e_clr = a + b * (-c)
    mask = vp < c
    e_clr = xr.where(mask, a - d * (vp - c), e_clr)
    return L_sb(e_clr, Ta)
...
def prata1996(Ta, RH):
    ...
    b = 3
    c = 0.5
    e_clr = 1 - (1 + prata1996_w(Ta, RH)) * np.exp(
        -1 * (a + b * prata1996_w(Ta, RH)) ** c
    )
    return L_sb(e_clr, Ta)


def satterlund1979(Ta, RH):
    """Satterlund (1979)"""
    a = 1.08
    b = 2016
    e_clr = (
        a * (1 - np.exp(-((10 * vap_pres(Ta, RH)) ** (Ta / b))))
    )  # note: corrected so that b paramter is in the exponent ^(Ta/b), error in Flerchinger Table 1
    return L_sb(e_clr, Ta)
...
def dilleyobrien1998(Ta, RH):
    ...
    c = 96.96
    L_clr = (
        a + b * (Ta / 273.16) ** 6 + c * np.sqrt(prata1996_w(Ta, RH) / 2.5)
...
def campbellnorman1998(Ta):
    ...
    a = (
        0.72 - 0.005 * 273.15
    )  # modified so Ta is in (K) instead of (C) ....should be -0.6458 after the maths...
    b = 0.005
    e_clr = a + b * Ta
    return L_sb(e_clr, Ta)
